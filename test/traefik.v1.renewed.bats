...
@test "check: when a certificate is renewed, the corresponding mailserver must receive and update the certificate" {
    ...
    run docker cp "$BATS_TEST_DIRNAME/fixtures/acme.v1.json" "${TEST_STACK_NAME}_mailserver-traefik_1":/tmp/traefik/acme.json
    assert_success
    run docker exec "${TEST_STACK_NAME}_mailserver-traefik_1" chmod 600 /tmp/traefik/acme.json
    assert_success
