...
@test "check: initial pull certificates when traefik was already running" {
    ...
    run repeat_until_success_or_timeout "$TEST_TIMEOUT_IN_SECONDS" sh -c "docker logs ${TEST_STACK_NAME_TRAEFIK}-traefik-1 | grep -F \"Got certificate for domains [traefik.localhost.com]\""
    assert_success
